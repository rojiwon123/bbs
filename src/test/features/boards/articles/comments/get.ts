...
const test = api.functional.boards.articles.comments.get;

export const test_get_comment_when_board_is_public_and_request_is_unauthorized =
    async (connection: IConnection) => {
        ...
        await APIValidator.assert(
            test(connection, board_id, article_id, comment_id),
            HttpStatus.OK,
        )({
            success: true,
...
export const test_get_comment_when_board_is_private_and_request_is_unauthorized =
    async (connection: IConnection) => {
...
export const test_get_comment_when_token_is_expired = async (
...
export const test_get_comment_when_token_is_invalid = async (
    connection: IConnection,
) => {
    const board_id = await Seed.getBoardId("board3");
    const article_id = await Seed.getArticleId(board_id);
...
export const test_get_comment_when_user_is_invalid = async (
    connection: IConnection,
) => {
    const username = "get_comment_when_user_is_invalid";
...
export const test_get_comment_when_board_does_not_exist = async (
    connection: IConnection,
) => {
...
export const test_get_comment_when_article_does_not_exist = async (
    connection: IConnection,
) => {
...
export const test_get_comment_when_article_is_deleted = async (
    connection: IConnection,
) => {
    const token = await get_token(connection, "user2");
    const board_id = await Seed.getBoardId("board2");
...
export const test_get_comment_when_comment_does_not_exist = async (
    connection: IConnection,
) => {
    const token = await get_token(connection, "user2");
    const board_id = await Seed.getBoardId("board2");
...
export const test_get_comment_when_comment_is_deleted = async (
    connection: IConnection,
) => {
    const token = await get_token(connection, "user2");
    const board_id = await Seed.getBoardId("board2");
...
export const test_get_comment_when_membership_is_insufficient = async (
    connection: IConnection,
) => {
    const token = await get_token(connection, "user1");
    const board_id = await Seed.getBoardId("board3");
